...
def print_travelled_distances():
    ...
    print("Total Distance by all three trucks: " + "{:.2f}".format(distance_travelled_list[3]))


# this function takes the user input from UI and displays the list of packages that are delivered
# and also the packages that are yet to be delivered in the given time
def package_details_at_particular_time(user_input):
    ...
    for item in range(1, package_hash.get_hashtable_size()):
        ...
        # comparing the time provided by the user to check the packages if it falls within that time
        if package.delivery_time <= user_input:
            # the time before it goes to the delivered package list
    ...
    print("Delivered package list by : " + str(user_input))
    for p in delivered_package_list:
        p.print_package_info()
    print("Packages not delivered by: " + str(user_input))
